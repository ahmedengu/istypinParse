...
app.post('/', function (req, res) {
    ...
    if (phoneNumber) {
        ...
        if (language == "en" && phoneNumber.length != 10
            ...
            res.render("index.ejs", extend(strings[language], {
                warning: strings[language]['warningPhone'],
                'language': language
            }));
        }
        Parse.Cloud.run("sendCode", {phoneNumber: phoneNumber, language: language}).then(function (response) {
            if (response) {
                ...
                res.render("index.ejs", extend(strings[language], {
                    warning: strings[language]['warningTwilio'],
                    'language': language
                }));
            }
        }, function (error) {
            res.render("index.ejs", extend(strings[language], {
                warning: strings[language]['warningGeneral'],
                'language': language
            }));
        });
    } else {
        res.render("index.ejs", extend(strings[language], {
            warning: strings[language]['warningNoNumber'],
            'language': language
        }))
    }
});
...
app.post('/verify', function (req, res) {
    ...
    if (code) {
        ...
        if (code.length != 4) {
            res.render("verify.ejs", extend(strings[language], {
                warning: strings[language]['warningCodeLength'],
                'language': language
            }));
        }
        ...
        res.render("verify.ejs", extend(strings[language], {
            phoneNumber: phoneNumber,
            warning: strings[language]['warningCodeInvalid'],
            'language': language
        }));
    }
});
